...
function getFullNames(runners) {
  ...
  runners.forEach(function(runners){
    runnerList.push(`${runners.last_name}, ${runners.first_name}`);
  });
  return runnerList;
}
...
function firstNamesAllCaps(runners) {
  const capFirst = [];
  let capFirstName = runners.map(function(runners){
    capFirst.push(`${runners.first_name.toUpperCase()}`);
  });
  return capFirst;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  return runners.filter(function(runner, idx){
...
function tallyUpDonations(runners) {
  return runners.reduce((accumulator, runner) => {
...
function counterMakerWithLimit(num) {
  let count = -1;
  return function() {
    return (count < num) ? count += 1 : count = 0;
  }
}
